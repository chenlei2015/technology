...
class Activity extends MY_Controller {
    ...
    private $_server_module = 'oversea/Activity/';
    ...
    {
        $api_name = $this->_server_module.strtolower(__FUNCTION__);
        $file_name = '海外活动列表_'.date('Ymd_H_i');
    ...
    public function templateDownload(){
        ...
        $file_name = "oversea_activity_template.csv";
        $filename = '海外活动量配置模板.csv';
    ...
    public function import()
    {
        ...
        $bind_required_cols_callback = [
            //站点
            'station_code' => function($col, &$line, $actual_col_position) {
                $platform_code = strtolower($line[$actual_col_position[$col]] ?? '');
                if (!isset(OVERSEA_STATION_CODE[$platform_code]))
                {
                    return false;
                }
                return true;
...
//        $bind_required_cols_extension_callback = [
//            //开始备货时间， 不能早于明天
//            'execute_purcharse_time' => function($col, &$line, $actual_col_position, $mode) {
//                $execute_purcharse_time = $line[$actual_col_position[$col]] ?? '';
//                $activity_start_time = $line[$actual_col_position['activity_start_time']] ?? '';
//                if (strtotime($execute_purcharse_time) > strtotime($activity_start_time)){
//                    return false;
//                }
//                return true;
//            },
