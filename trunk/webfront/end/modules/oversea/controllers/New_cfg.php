...
class New_cfg extends MY_Controller{
    private $_server_module = 'oversea/New_cfg/';
    ...
    public function import()
    {
        ...
        foreach (OVERSEA_STATION_CODE as $key =>$value){
            $oversea_site[$value['name']] = $key;
        }

        //平台映射
        foreach (INLAND_PLATFORM_CODE as $key =>$value){
            $platform[$value['name']] = $key;
        }
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            foreach ($chunkData[$i] as $k => $value) {
                ...
                if(empty($oversea_site[$site_lower]))
                {
                    array_push($err_info['errorLines'],$line_num);
                    continue;
                }
                $platform_name = $v[$final_list['platform']];
                ...
                if(preg_match('/^[+-]?[\d]+([\.][\d]+)?([Ee][+-]?[\d]+)?$/', $v[$final_list['sku']]))
                {
                    array_push($err_info['errorLines'],$line_num);
                    continue;
                }
                foreach ($final_list as $key => $val) {
                    ...
                    if($key == 'site')
                    {
                        $row[$key] = $oversea_site[$site_lower];
                    }
                    if($key == 'platform')
                    {
                        $row[$key] = $platform[$platform_name];
                    }
                }
    ...
    public function templateDownload()
    {
        ...
        $file_name = "oversea_new_template.csv";
        $filename = '海外新品和首发量配置模板.csv';
