...
class FbaDebugRebuildPr
{
    ...
    public function run($version, $pr_sns)
    {
        $this->hook('before');
        ...
        $version_info = ['version' => $version, 'processed' => 0, 'total' => count($pr_sns), 'offset' => 0];
        ...
        $recalc_pr = $this->get_rebuild_pr_by_gid($pr_sns);
        //pr($recalc_pr);exit;
        ...
        $batch_update = $batch_insert_log = $aggr_ids = $listing_aggr_ids = $aggr_new_ids = $lost_python_ids = [];
            ...
            foreach ($recalc_pr as $key => &$row)
            {
                if ($row['approve_state'] != APPROVAL_STATE_FIRST) {
                ...
                pr(array_keys($this->column_callback));
    ...
    protected function get_rebuild_pr_by_gid($pr_sns)
    {
        return $this->model->get_debug_rebuild_by_sns($pr_sns);
    }
