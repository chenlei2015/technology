...
class PrState extends OrderState
{
    ...
    public function actions($extend_params = []) : array
    {
        $base = [
            'view' => [
                'name' => 'view',
                'url'  => $extend_params['site_url'].'/fba/pr/view/'.$extend_params['gid']                    
            ]
        ];
        ...
        if ($this->can_action('edit'))
        {
            $edit = [
                'edit' => [
                    'name' => 'edit',
                    'url'  => $extend_params['site_url'].'/fba/pr/edit/'.$extend_params['gid']
                ]
            ];
        }
        ...
        if ($this->can_action('remark'))
        {
            $remark = [
                    'remark' => [
                            'name' => 'remark',
                            'url'  => $extend_params['site_url'].'/fba/pr/remark/'.$extend_params['gid']
                    ]
            ];
        }
    ...
    private final function _set_default()
    {
        return [
                ...
                APPROVAL_STATE_FIRST => [APPROVAL_STATE_SECOND, APPROVAL_STATE_FAIL,APPROVAL_STATE_SUCCESS],
                APPROVAL_STATE_SECOND => [APPROVAL_STATE_THREE, APPROVAL_STATE_FAIL],
                APPROVAL_STATE_THREE => [APPROVAL_STATE_SUCCESS, APPROVAL_STATE_FAIL],
    ...
    private final function _set_state_action()
    {
        return $this->_state_action_map = [
                ...
                'edit_pr_listing' => (
                    function($params) {
                        return in_array($params['approve_state'], [APPROVAL_STATE_NONE, APPROVAL_STATE_FIRST, APPROVAL_STATE_FAIL]);  
                }),
        ];
    }
