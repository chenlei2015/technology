...
class FbaApproveFirst
{
    ...
    protected function _get_next_state($result, $is_plan_approve, $is_first_sale)
    {
    ...
    public function recive($todo)
    {
        ...
        $this->_ci->load->classes('fba/classes/PrState');
    ...
    public function update_approve_succ()
    {
        ...
        try
        {
            ...
            $this->_ci->load->service('fba/PrTrackLogService');

            $this->_ci->load->model('Fba_pr_list_log_model', 'fba_pr_list_log', false, 'fba');
            $my_db = $this->_ci->fba_pr_list_log->getDatabase();
            ...
            {
                //更新汇总
                $this->_ci->load->service('fba/PrSummaryService');
    ...
    protected function update_approve_fail()
    {
        ...
        try
        {
            ...
            //批量更新状态,主被动都在这里
            if (!$this->_other_db->update_batch_more_where($this->_model->getTable(), $this->_batch_update, 'gid'))
            {
                throw new \RuntimeException(sprintf('执行%s级审核操作，批量更新失败', $this->_level), 500);
            }
