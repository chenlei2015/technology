...
class FbaActivityHugeExport extends AbstractHugeExport
{
    private $_db;
    ...
    public function __construct()
    {
        ...
        $this->_db = $this->_ci->m_fba_activity->getDatabase();
    }
    ...
    protected function get_default_template_cols()
    {
        ...
        $join_cols =  ['staff_zh_name' => '创建人', 'updated_zh_name' => '修改人', 'approved_zh_name' => '审核人'];
        foreach ($this->_ci->usercfgprofileservice->export_temple('fba_activity') as $title => $cfg)
        {
        ...
        try
        {
            ...
            else
            {
                $trans = function($row) use ($pick_cols, $col_map) {
                    $new = [];
                    $cols = ['created_at', 'updated_at', 'approve_at', 'activity_start_time', 'activity_end_time', 'execute_purcharse_time'];
                                ...
                                if ($col == "activity_state"){
                                    switch ($row['activity_state']){
                                        case 1:
                                            ...
                                            }elseif ($row['activity_start_time'] < date("Y-m-d H:i:s",time()) && $row['activity_end_time'] > date("Y-m-d H:i:s",time())) {
                                                $new[$col] .= $col_map[$col][ACTIVITY_STATE_ING]['name'] ?? $row[$col];
                                            }elseif ($row['activity_end_time'] < date("Y-m-d H:i:s",time())){
