...
class EstimateMoqPurchaseMoney
{
    ...
    {
        property_exists($this->ci, 'm_inland_pr_list') OR $this->ci->load->model('Inland_pr_list_model', 'm_inland_pr_list', false, 'inland');

        list($start_gid, $end_gid) = $this->ci->m_inland_pr_list->get_top_bottom_gid();
        ...
        $sql = sprintf('
            SELECT
				...
				if(tmp.moq_qty - tmp.orign_require_qty <= tmp.max_sp * tmp.weight_sale_pcs , tmp.moq_qty, 0)) as final_require_qty,
		 ...
		 from (
			select
				...
				@weight_sale_pcs = sum(list.weight_sale_pcs) as weight_sale_pcs,
					...
					sum(IF(list.require_qty <= 0, 0, list.require_qty) - list.available_qty - list.ship_qty - if(t_pr.pr_qty, t_pr.pr_qty, 0)) as orign_require_qty
				from yibai_inland_pr_list list
				left join (
					select pr.sku, pr.is_refund_tax, pur.purchase_warehouse_id, sum(pr.push_stock_quantity) as pr_qty
					from yibai_plan_common.yibai_pr_number pr
				...
				and list.is_trigger_pr = %d
				and list.require_qty > 0
				and list.purchase_price > 0
				and list.gid >= "%s" and list.gid <= "%s"
				and IF(list.require_qty <= 0, 0, list.require_qty) - list.available_qty - list.ship_qty - if(t_pr.pr_qty, t_pr.pr_qty, 0) > 0
		...
		if(tmp.moq_qty - tmp.orign_require_qty <= tmp.max_sp * tmp.weight_sale_pcs , tmp.moq_qty, 0)) > 0
        order by tmp.supplier_code asc',
            BUSSINESS_IN, GLOBAL_YES,
            INLAND_APPROVAL_STATE_INIT,SKU_STATE_DOWN,SKU_STATE_CLEAN, TRIGGER_PR_YES,
            $start_gid, $end_gid
       );
        ...
        $db = $this->ci->m_inland_pr_list->getDatabase();
    ...
    private function execute_supplier_moq_check($supplier_skus, &$insert_purchase_sku_info)
    {
        ...
        property_exists($this->ci, 'm_sku_cfg') OR $this->ci->load->model('Fba_sku_cfg_model', 'm_sku_cfg', false, 'inland');
    ...
    {
        property_exists($this->ci, 'm_estimate_fba') OR $this->ci->load->model('Estimate_fba_moq_purchase_list_model', 'm_estimate_fba', false, 'inland');
    ...
    {
        property_exists($this->ci, 'm_rebuild_mvcc') OR $this->ci->load->model('Fba_rebuild_mvcc_model', 'm_rebuild_mvcc', false, 'inland');
    ...
    {
        property_exists($this->ci, 'm_rebuild_mvcc') OR $this->ci->load->model('Fba_rebuild_mvcc_model', 'm_estimate_fba', false, 'inland');
