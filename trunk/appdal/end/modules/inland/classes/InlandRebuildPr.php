...
class InlandRebuildPr
{
     ...
     * @return CsvReader
     */
    protected function init_env_resource() : InlandRebuildPr
    {
        set_time_limit(-1);
        ...
        $this->ci->load->model('Inland_rebuild_mvcc_model', 'm_rebuild_mvcc', false, 'inland');
        $this->ci->load->model('Inland_rebuild_backup_sku_model', 'm_rebuild_backup_sku', false, 'inland');
        $this->ci->load->model('Inland_python_cfg_model', 'm_python_cfg', false, 'inland');
        $this->ci->load->model('Inland_exception_list_model', 'm_exception_list', false, 'inland');

        return $this;
    }

    /**
     * @param Inland_pr_list_model $model
     * @return FbaRebuildPr
     */
    ...
    public function run()
    {
        ...
        while ( ($start_gid = $this->sampling($lock_key, $pool_key)) && (true !== $start_gid) && ($recalc_pr = $this->get_rebuild_pr_by_gid($chunk_size, $start_gid)))
        {
            ...
            $batch_update = $batch_insert_log = $aggr_ids = $listing_aggr_ids = $lost_python_ids = [];
            ...
            foreach ($recalc_pr as $key => &$row)
            {
                if ($row['approve_state'] != INLAND_APPROVAL_STATE_INIT || $row['version'] == $version) {
        ...
        $this->register_callback('after', function($params)  {
            ...
            return $ci->Task->delivery(['inland', 'PR', 'asyc_estimate_rebuild_money'], [$params['version']]);
        }, ['version' => $version]);
    ...
    protected function create_rebuild_mvcc_and_backup()
    {
        ...
        $add_result = $this->ci->m_rebuild_mvcc->add(BUSSINESS_IN);
        //新增
        ...
        if (!empty($this->inspect_callback[$pos])) {
            foreach ($this->inspect_callback[$pos] as $cb)
            {
                if (isset($cb['cb']) && is_callable($cb['cb'])) {
                    ...
                    log_message('INFO', sprintf('InlandRebuildPr.hook %s调用返回结果：%s', $pos, json_encode($result)));
                }
            }
        }
        return $result;
