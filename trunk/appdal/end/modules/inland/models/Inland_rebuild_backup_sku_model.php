 ...
 * @link
 */
class Inland_rebuild_backup_sku_model extends MY_Model
{
    ...
    {
        $this->database = 'stock';
        $this->table_name = 'yibai_inland_rebuild_backup_sku_cfg';
    ...
    {
        $insert_sql = 'INSERT INTO yibai_inland_rebuild_backup_sku_cfg (version,aggr_id,sku,max_safe_stock_day,reduce_factor,bs,sp,shipment_time,sc,first_lt,sz,stock_way)
        select '.$version.',
        ...
        left join yibai_inland_sku_cfg_history t_sk_his on t_sk.gid = t_sk_his.gid and t_sk.state != 2,
        (select @bs:=bs, @sp:=sp, @shipment_time:=shipment_time, @sc:=sc, @first_lt:=first_lt, @sz:=sz from yibai_inland_global_rule_cfg limit 1) as  gcfg
        ';
