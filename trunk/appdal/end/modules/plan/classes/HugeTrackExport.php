...
class HugeTrackExport extends AbstractHugeExport
{
    ...
    public function __construct()
    {
        ...
        $this->_ci->load->model('Plan_purchase_track_list_model', 'purchase_track_list', false, 'plan');
        $this->_db = $this->_ci->purchase_track_list->getDatabase();
    }
    ...
    {
        $this->_ci->load->classes('plan/classes/TrackTemplate');
    ...
    public function set_translator() : AbstractHugeExport
    {
        ...
        $this->col_map = [
            ...
            'po_state' => PURCHASE_ORDER_STATUS,
        ...
        try
        {
            ...
            else
            {
                $trans = function($row) use ($pick_cols, $col_map) {
                    $new = [];
                    $cols = ['created_at', 'updated_at','stockin_date','qt_date','freight_date','exchange_signed_date,earliest_exhaust_date'];
                    foreach ($pick_cols as $col)
                    {
                        //丢弃gid
                        if ($col == 'gid')
