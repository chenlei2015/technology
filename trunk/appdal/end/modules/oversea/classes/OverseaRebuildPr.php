...
//            'weight_sale_pcs',
     ...
     * @return CsvReader
     */
    protected function init_env_resource() : OverseaRebuildPr
    {
        set_time_limit(-1);
        ...
        $this->ci->load->model('Oversea_rebuild_mvcc_model', 'm_rebuild_mvcc', false, 'oversea');
        $this->ci->load->model('Oversea_rebuild_backup_sku_model', 'm_rebuild_backup_sku', false, 'oversea');
        $this->ci->load->model('Oversea_python_cfg_model', 'm_python_cfg', false, 'oversea');
        $this->ci->load->model('Oversea_exception_list_model', 'm_exception_list', false, 'oversea');

        return $this;
    }

    /**
     * @param Oversea_pr_list_model $model
     * @return OverseaRebuildPr
     */
...
//        }
        ...
        //初始化gid
        $gids = $this->model->sampling($this->model->get_rebuild_top_bottom_gid());
    ...
    public function run()
    {
        ...
        $reached_virtual_warehouse = false;
...
//var_dump($this->sampling($lock_key, $pool_key));die;

        while ( ($start_gid = $this->sampling($lock_key, $pool_key)) && (true !== $start_gid) && ($recalc_pr = $this->get_rebuild_pr_by_gid($chunk_size, $start_gid)))
        {
            ...
            $batch_update = $batch_insert_log = $aggr_ids = $listing_aggr_ids = [];

            //开始还原php版本的参数
            foreach ($recalc_pr as $key => &$row)
            {
                //重新生成需求列表时，采购仓库为平台头程虚拟仓-塘厦的记录，不用重新生成，需求数量和之前保持一致。
                if ($row['purchase_warehouse_id'] == PURCHASE_WAREHOUSE_TCXNC) {
                    $reached_virtual_warehouse = true;
                    continue;
                }
                ...
                $row['aggr_id'] = md5(trim($row['sku']).$row['is_refund_tax'].$row['purchase_warehouse_id']);
            ...
            if ($this->enable_python)
            {
                /* @var Oversea_python_cfg_model $python_cfg */
                /* @var string aggr_id $listing_aggr_ids */
            ...
            foreach ($recalc_pr as $key => &$row)
            {
                ...
                if ($row['approve_state'] != OVERSEA_APPROVAL_STATE_INIT || $row['version'] == $version) {
                    log_message('INFO', sprintf('pid:%d  version:%d，第%d次取值，该记录不能处理或已处理：状态：%d, 版本：%d。', $pid, $version, $nums, $row['approve_state'], $version));
                    continue;
                }
                $log_context = [];
                ...
                {
                    log_message('ERROR', sprintf('pid:%d version:%d，第%d次取值，无法获取pr_sn:%s计划配置,aggr_id:%s, 参数：%s 值：%s', $pid, $version, $nums, $row['pr_sn'], $row['aggr_id'], 'sku,is_refund_tax, purchase_warehouse_id', implode(',', [trim($row['sku']), $row['is_refund_tax'],$row['purchase_warehouse_id']])));
                    $log_context[] = '该记录无法找到备份配置，没有重新计算';
                    continue;
                    //$batch_update[] = $tmp;
                }
                if ($this->enable_python && !isset($python_cfg[$row['py_diff_aggr_id']]))
                {
                    log_message('ERROR', sprintf('pid:%d version:%d，第%d次取值，无法获取pr_sn:%s PYTHON配置,aggr_id:%s, 参数：%s 值：%s, 需求设置0', $pid, $version, $nums, $row['pr_sn'], $row['aggr_id'], 'sku,is_refund_tax, purchase_warehouse_id', implode(',', [trim($row['sku']), $row['is_refund_tax'],$row['purchase_warehouse_id']])));
                    $log_context[] = '该记录无法找到python配置，需求设置0';
                    $row['lost_python_cfg'] = true;
            ...
            try
            {
                $common_db->trans_start(true);
                $stock_db->trans_start(true);
        ...
        if (true === $start_gid) {
            ...
            log_message('INFO', sprintf('pid:%d 重建需求版本：%d 执行结束异常分支，起始start_gid: %s 当前最大gid:%s, 我的最大gid：%s 我不是最后一个进程，不做处理退出', $pid, $version, $start_gid, $max_gid, $my_max_gid));
        }
    ...
    protected function create_rebuild_mvcc_and_backup()
    {
        ...
        $add_result = $this->ci->m_rebuild_mvcc->add(BUSSINESS_OVERSEA);
        //新增
        ...
        if (!empty($this->inspect_callback[$pos])) {
            foreach ($this->inspect_callback[$pos] as $cb)
            {
                if (isset($cb['cb']) && is_callable($cb['cb'])) {
                    ...
                    log_message('INFO', sprintf('OverseaRebuildPr.hook %s调用返回结果：%s', $pos, json_encode($result)));
                }
            }
        }
        return $result;
