...
class OverseaHugeExport extends AbstractHugeExport
{
    ...
    public function __construct()
    {
        ...
        $this->_db = $this->_ci->oversea_pr_list->getDatabase();
    }
    ...
    {
        $this->_ci->load->classes('oversea/classes/ListTemplate');
        return $this->_ci->ListTemplate->get_default_template_cols();
    }
    ...
    public function run()
    {
        try
        {
            ...
            $file_name = '海外站点需求列表_'.date('YmdHi');
            ...
            else
            {
                $trans = function($row) use ($pick_cols, $col_map) {
                    ...
                    foreach ($pick_cols as $col)
                    {
                        ...
                        //是否精品
                        if ($col == 'is_boutique'){
                            $new[$col] = isset(INLAND_QUALITY_GOODS[$row[$col]])?INLAND_QUALITY_GOODS[$row[$col]]['name']:"--";
                            continue;
                        }
                        //erp系统sku状态
