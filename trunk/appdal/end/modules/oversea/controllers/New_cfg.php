...
class New_cfg extends MY_Controller {
    ...
    public function list()
    {
        try
        {
            ...
            $this->load->service('oversea/NewCfgListService');
    ...
    public function batch_approve()
    {
        try
        {
            ...
            $this->load->service('oversea/NewCfgService');
    ...
    public function export()
    {
        try
        {
            ...
            $this->load->service('oversea/NewCfgExportService');
    ...
    /**
     * 添加备注
     * oversea/new_cfg/remark_add
     */
    public function remark_add()
    {
        $this->load->model('Oversea_new_remark_model', "m_new_r");
    ...
    /**
     * 查看-基本信息
     * oversea/new_cfg/base_info
     */
    public function base_info()
    {
        $this->lang->load('common');
        $this->_ci->load->model('Oversea_new_list_model', 'm_over_new', false, 'fba');
        $id = $this->input->post_get('id') ?? '';
        $result = $this->m_over_new->get_base_info($id);
        $this->data['status'] = 1;
        $this->data['data_list'] = [
            'value' => $result??[],
        ];
    ...
    /**
     * 查看-备注
     * oversea/new_cfg/remark
     */
    public function remark()
    {
        $this->lang->load('common');
        $this->_ci->load->model('Oversea_new_remark_model', 'm_over_r', false, 'oversea');
        ...
        $result = $this->m_over_r->get_remark('',$new_id);
        $this->data['status'] = 1;
        $this->data['data_list'] = [
            'key'   => $column_keys,
            'value' => $result??[],
    ...
    {
        $this->lang->load('common');
        $this->_ci->load->model('Oversea_new_log_model', 'm_over_n_l', false, 'fba');
        $id = $this->input->post_get('id') ?? '';
        ...
        $result = $this->m_over_n_l->getStockLogList($id, $offset, $limit);
        $this->data['status'] = 1;
    ...
    /**
     * 修改
     * oversea/new_cfg/modify
     */
    public function modify()
    {
        $this->load->model('Oversea_new_list_model', "m_n_l");
        $id = $this->input->post_get('id') ?? '';
    ...
    /**
     * 备货详情
     * oversea/new_cfg/stock
     */
    public function stock()
    {
        $id = $this->input->post_get('id') ?? '';
        $this->_ci->load->model('Oversea_stock_info_model', 'm_fba_s_i', false, 'fba');
