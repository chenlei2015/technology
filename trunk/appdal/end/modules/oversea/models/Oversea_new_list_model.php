 ...
 * @link
 */
class Oversea_new_list_model extends MY_Model
{
    private $_ci;
    ...
    public function __construct()
    {
        ...
        $this->table_name = 'yibai_oversea_new_cfg';
        parent::__construct();
    }
    ...
    {
        //md5(站点+平台+sku)
        $id = md5(strtolower(trim($data['site'])).trim($data['platform']).trim($data['sku']));
        return $id;
    }
    ...
    {
        $this->db->select('id,approve_state,demand_num,is_delete');
        $this->db->from($this->table_name);
    ...
    public function modify($params = [], $user_id = '', $user_name = '')
    {
        try {
            ...
            $insert_log = [
                ...
                'uid' => $user_id,
                'user_name' => $user_name,
                'context' => "海外新品修改成功,原需求数量:{$new_info['demand_num']},修改后需求数量:{$params['demand_num']}",
            ];
            $this->_ci->load->model('Oversea_new_log_model', 'oversea_new_log', false, 'oversea');
            $this->_ci->oversea_new_log->add($insert_log);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                return false;
            } else {
