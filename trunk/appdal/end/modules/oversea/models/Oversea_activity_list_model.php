 ...
 * @link
 */
class Oversea_activity_list_model extends MY_Model
{
    ...
    {
        $this->database = 'stock';
        $this->table_name = 'yibai_oversea_activity_cfg';
        $this->primaryKey = 'id';
        parent::__construct();
    }
    ...
    public function get_future_valid_activities()
    {
        ...
        $result = $this->_db->from($this->table_name)
        ->select('erpsku,amount,activity_start_time,activity_end_time, execute_purcharse_time')
        ->where('approve_state', ACTIVITY_APPROVAL_SUCCESS)
        ->where('activity_start_time >', $now)
        ->where('activity_state !=', ACTIVITY_STATE_DISCARD)
