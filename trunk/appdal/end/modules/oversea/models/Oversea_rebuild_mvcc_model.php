 ...
 * @link
 */
class Oversea_rebuild_mvcc_model extends MY_Model
{
    ...
    {
        $this->database = 'stock';
        $this->table_name = 'yibai_oversea_rebuild_mvcc';
    ...
    {
        //检测文件
        if (!file_exists(APPPATH . 'upload/oversea_cfg.php')){
            throw new \RuntimeException('oversea_cfg.php'."配置文件不存在");
        }
        require APPPATH . 'upload/oversea_cfg.php';//引入文件
        $oversea_cfg = compact(['sales_amount_cfg', 'exhaust_cfg', 'in_warehouse_age_cfg', 'supply_day_cfg',  'sale_category_cfg']);
        //变量检测
        foreach (['sales_amount_cfg', 'exhaust_cfg', 'in_warehouse_age_cfg', 'supply_day_cfg',  'sale_category_cfg'] as $k => $v){
            if(!isset($oversea_cfg[$v])){
                throw new \RuntimeException('rebuild_cfg.php'."配置文件中缺少：".$v);
            }
        }
        $oversea_cfg = json_encode($oversea_cfg);
        ...
        {
            $ci = CI::$APP;
            $ci->load->model('Oversea_pr_list_model', 'oversea_pr_list', false, 'oversea');
            ...
            $insert = [
                    ...
                    'total' => $ci->oversea_pr_list->get_today_pr_total(),
                    'business_line' => $business_line,
                    'state' => REBUILD_CFG_BACKUP_OVER,
                    'processed' => 0,
                    'offset' => 0,
                    'strategy' => $oversea_cfg,
            ];
    ...
    public function clean()
    {
        ...
        $clean_sql = sprintf('delete from %s where version in (%s)', 'yibai_oversea_rebuild_backup_sku_cfg', implode(',', $delete_version));
