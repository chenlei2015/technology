...
class PrLogService
{
    ...
    public function send($params, $context)
    {
        ...
        $row = $this->_ci->m_inland_pr_list_log->fetch_table_cols($params);
        return $this->_ci->m_inland_pr_list_log->add($row);
    }
    ...
    public function multi_send($params)
    {
        ...
        return $this->_ci->m_inland_pr_list_log->madd($params);
    }
    
    public function get_one_listing_log($gid, $offset = 1, $limit = 20)
    {
        $total = 0;
        $rows = $this->_ci->m_inland_pr_list_log->get($gid, $total, $offset, $limit);
