...
class PrSummaryListService extends AbstractList
{
    ...
    public function quick_export($gids = '', $profile = '*', $format_type = EXPORT_VIEW_PRETTY, $data_type = VIEW_BROWSER, $charset = 'UTF-8' )
    {
        ...
        if ($gids != '')
        {
            ...
            $quick_sql = $db->from($this->_ci->m_inland_summary->getTable())->where_in('gid', $gids_arr)->order_by(self::$_default_sort)->get_compiled_select('', false);
            $db->reset_query();
        }
        else
        {
            ...
            $quick_sql = $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::INLAND_PR_SUMMARY_SEARCH_EXPORT)->get();
        ...
        $this->_ci->load->classes('inland/classes/InlandHugeSummaryExport');
        $this->_ci->InlandHugeSummaryExport
        ...
        return $this->_ci->InlandHugeSummaryExport->run();
    }
    ...
    public function get_cfg() : array
    {
        ...
        $this->_cfg = [
            ...
            'select_cols' => [
                    ($this->_ci->m_inland_summary->getTable()) => [
                        '*'
                    ],
            ],
            'search_rules' => &$search,
            'droplist' => ['purchase_warehouse'],
            'user_profile' => 'yibai_inland_summary_cfg',
        ];
        return $this->_cfg;
    }
    ...
    protected function search($params)
    {
        ...
        $o_t = $this->_ci->m_inland_summary->getTable();
        ...
        $page = ceil($count / $params['per_page']);
         ...
         if (isset($params['export_save']))
         {
             ...
             $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::INLAND_PR_SUMMARY_SEARCH_EXPORT)->set($total.($query_export->get_compiled_select('', false)));
         }
