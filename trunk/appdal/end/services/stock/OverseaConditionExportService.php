...
class OverseaConditionExportService
{
    /**
     * client
     *
    ...
    {
        if (empty($post['time_type']) || !in_array($post['time_type'],array(1,2,3)))
        {
            throw new \InvalidArgumentException(sprintf('time_type参数错误'), 3001);
        }

        $this->_template = $post['template'] ?? __CLASS__.'_'.$post['time_type'];
     ...
     * @param unknown $post
     */
    public function export($file_type = 'csv',$time_type)
    {
        ...
        $this->_ci->load->service('stock/StockConditionService');
        return $this->_ci->stockconditionservice->quick_export_oversea($this->_gids, $this->_profile, $this->_format_type,$time_type,$this->_data_type, $this->_charset);
