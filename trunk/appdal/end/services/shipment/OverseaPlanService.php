...
class OverseaPlanService
{
    ...
    /**
     * @ TODO
     * 默认发运数量: 拿可调拨库存 根据'sku相同的按最早缺货时间按顺序' 分配默认发运数量
     * 可调拨库存: 调接口获取,http://192.168.71.156/web/#/87?page_id=3446
     * 发运类型: 待java确认
     * 采购成本: 采购单价*发运数量 = 采购成本, 采购单价字段待java确认
     * @param $params
     */
    public function planByTracking($params)
    {
        ...
        $this->_ci->load->model('Oversea_pr_track_list_model', 'm_pr_track', false, 'oversea');
        $this->_ci->load->model('Oversea_pr_list_model', 'm_pr', false, 'oversea');
        ...
        $final_data = $db->select('*')->from('yibai_shipment_detail_oversea_temp')->where('track_time',$track_date)->get()->result_array();
        ...
        //sku相同
        $all_sku = array_unique(array_column($final_data,'sku'));

        foreach ($final_data as $key => &$item)
        {
            //采购成本 调java接口
            $item['pur_cost'] = bcmul($purchase_price[$item['sku']]['price'],$item['pr_qty'],2);


            //可调拨库存
//            if($item['is_refund_tax'] == 1){//退税
//                $item['available_inventory'] = $avail_qty[$item['sku']]['taxAvailQty']??0; //100临时测试
...
//            }
    ...
    public function once_add_shipment($params)
    {
        ...
        define('OVERSEA_UNIQUE_SHIPMENT','OVERSEA_UNIQUE_SHIPMENT'.$date_info['start']);
        //通过业务线+日期
        $this->_ci->load->library('rediss');

        if ($this->_ci->rediss->getData(OVERSEA_UNIQUE_SHIPMENT)){
            $this->data['status']    = 0;
            $this->data['errorMess'] = '服务器繁忙,请稍后重试';
            http_response($this->data);
        }
        $this->_ci->rediss->setData(OVERSEA_UNIQUE_SHIPMENT,1,3600);
        ...
        $this->_ci->load->model('Oversea_shipment_list_model', 'm_shipment_plan', false, 'shipment');
        $this->_ci->load->model('Processed_date_model', 'm_processed_date', false, 'basic');
        $count = $this->_ci->m_pr_track->dataCount($date_info);
        $date_info['limit'] = 500;
        //分批查询
        if (empty($count)){
            $this->_ci->rediss->deleteData(OVERSEA_UNIQUE_SHIPMENT);
            throw new \RuntimeException(sprintf('数据为空'), 500);
        }
        $pr_info = [];
        $data =[];
        $shipment_sn   = $this->general_shipment_sn('oversea_shipment');//发运计划编号
    ...
    public function add_plan($shipment_sn,$track_date)
    {
        ...
        $data = [
            ...
            'business_line' => BUSINESS_LINE_OVERSEA,
            'date' => $track_date,
            'sn' => $shipment_sn,
    ...
    public function build_data($pr_info,$shipment_sn){
        ...
        foreach ($pr_info as $key => $item){
            $skus[]=[
                ...
                'businessLine' => BUSINESS_LINE_OVERSEA
            ];
        }
        ...
        foreach ($pr_info as $key => $item) {
            ...
            $final_data[] = [
                ...
                'stock_sn'      => $item['pur_sn'],//备货单号 需求跟踪列表
                'pr_sn'         => $item['pr_sn'],
                'pur_sn'        => $pur_sns[$item['pur_sn']]['po_sn']??'',//采购单号  备货跟踪列表
                'sku'           => $item['sku'],
                'shipment_type' => $pur_sns[$item['pur_sn']]['shipment_type']??'',//发运类型  备货跟踪列表
                'business_type' => BUSINESS_TYPE_OVERSEA,//写死海外仓
                //物流类型  站点需求列表
                'logistics_id'  => $logistics_id,
                'station_code'           => $item['station_code'],
                'is_refund_tax'          => $item['is_refund_tax'],
     ...
     * @TODO
     * 提前处理数据
     * 数据: 除了(发运类型采购成本,可用库存,默认发运数量)字段没有保存,
     * 另外保存字段: 最早缺货时间(时间戳)用于计算出默认发运数量
     * @param $params
     *
     * @return bool
     */
    public function pre_data($params)
    {
        ...
        if(empty($pr_info)){
            log_message('ERROR', sprintf('生成发运计划详情失败,接口:shipment/Plan/oversea_pre_data,失败原因:查询需求跟踪列表返回数据为空,开始时间%s结束时间%s',$date['start'],$date['end']));
        }

        $shipment_sn   = $this->general_shipment_sn('oversea_shipment');//生成发运计划编号
        $pr_sn_arr     = array_filter(array_column($pr_info, 'pr_sn'));//需求单号
        $pur_sn_arr    = array_filter(array_column($pr_info, 'pur_sn'));//备货单号
        $pur_sns       = [];
        $logistics_ids = [];
        ...
        foreach ($pr_info as $key => $item) {
            ...
            $final_data[] = [
                ...
                'pr_sn'                  => $item['pr_sn'],
                //采购单号  备货跟踪列表
                'pur_sn'                 => $pur_track_info[$item['pur_sn']]['po_sn']??'',
                'sku'                    => $item['sku'],
                //发运类型
                'business_line'          => 2,//写死海外
                //物流类型  站点需求列表
                'logistics_id'           => $logistics_ids[$item['pr_sn']]??'',
                'station_code'           => $item['station_code'],
                'is_refund_tax'          => $item['is_refund_tax'],
                ...
                'track_time'             => date('Y-m-d', $date['start']),
        ...
        $db->trans_start();
        if (!empty($final_data)){
            $count = $db->insert_batch('yibai_shipment_detail_oversea_temp', $final_data);
        }
        $db->trans_complete();
        if ($db->trans_status() === FALSE) {
            log_message('ERROR',sprintf('生成发运计划详情失败,接口:shipment/Plan/oversea_pre_data,失败原因:trans_status=false'));
            return FALSE;
        } else {
            log_message('INFO',sprintf('生成发运计划详情成功,接口:shipment/Plan/oversea_pre_data,开始时间%s结束时间%s,插入数量$count=%s',$date['start'],$date['end'],$count));
            return TRUE;
        }
    }
    ...
    public function upload($batch_params)
    {
        ...
        $this->_ci->load->model('Oversea_shipment_list_model', 'm_shipment_plan', false, 'shipment');
        $this->_ci->load->model('Oversea_list_model', 'shipment_detail', false, 'shipment');
...
//        $avail_qty = $this->get_available_inventory($skus);
        ...
        foreach ($batch_params as $key => &$item)
        {
            ...
            $item['stock_sn']    = $this->general_shipment_sn('oversea_pur');//备货单号
            $item['pr_sn']       = $this->general_shipment_sn('oversea_pr');//需求单号
            //采购单号  调接口
            $item['pur_sn']                 = $purchase_price[$item['sku']]['purchaseNumber']??'';//采购单号
            ...
            $item['order_source'] = '海外';
            $item['country_code'] = STATION_COUNTRY_MAP[$item['station_code']]['code']??'';
            $item['warehouse_code'] = $warehouse_map[$item['warehouse_id']]??'';
            $item['logistics_code'] = LOGISTICS_ATTR[$item['logistics_id']]['code']??'';

//            $item['is_refund_tax'] = $drawback_info[$item['sku']];已废弃
...
//            }
    ...
    public function auto_push_logistics($shipment_sn)
    {
        ...
        $data['businessLine'] = BUSINESS_LINE_OVERSEA;
        //推送至物流系统
...
//                ]];
    ...
    /**
     * 获取可用库存接口
     * http://192.168.71.156/web/#/87?page_id=3446
     * @param $skus
     *
     * @return array
     */
