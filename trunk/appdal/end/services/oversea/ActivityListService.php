...
class ActivityListService extends AbstractList
{
    ...
    public function quick_export($gids = '', $profile = '*', $format_type = EXPORT_VIEW_PRETTY, $data_type = VIEW_BROWSER, $charset = 'UTF-8' )
    {
        ...
        if ($gids != '')
        {
            ...
            $quick_sql = $db->from($this->_ci->m_oversea_activity->getTable())->where_in('id', $gids_arr)->order_by(self::$_default_sort)->get_compiled_select('', false);
            $db->reset_query();
        }
        else
        {
            ...
            $quick_sql = $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::OVERSEA_ACTIVITY_SEARCH_EXPORT)->get();
        ...
        $this->_ci->load->classes('oversea/classes/OverseaActivityHugeExport');
        $this->_ci->OverseaActivityHugeExport
        ...
        return $this->_ci->OverseaActivityHugeExport->run();
    ...
    public function get_cfg() : array
    {
        ...
        $this->_cfg = [
            ...
            'select_cols' => [
                ($this->_ci->m_oversea_activity->getTable()) => [
                    '*'
                ],
            ],
            'search_rules' => &$search,
            'droplist' => [
                'oversea_activity_approve_state', 'oversea_activity_state', 'oversea_platform_code','os_station_code'
            ],
            'user_profile' => 'oversea_activity'
        ];
        return $this->_cfg;
    }
    ...
    protected function search($params)
    {
        ...
        $o_t = $this->_ci->m_oversea_activity->getTable();
        ...
        if (isset($params['export_save']))
        {
            ...
            $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::OVERSEA_ACTIVITY_SEARCH_EXPORT)->set($total.($query_export->get_compiled_select('', false)));
        }
