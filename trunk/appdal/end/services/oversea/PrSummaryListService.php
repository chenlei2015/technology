...
class PrSummaryListService extends AbstractList
{
    ...
    public function quick_export($gids = '', $profile = '*', $format_type = EXPORT_VIEW_PRETTY, $data_type = VIEW_BROWSER, $charset = 'UTF-8' )
    {
        ...
        if ($gids != '')
        {
            ...
            $quick_sql = $db->from($this->_ci->oversea_summary->getTable())->where_in('gid', $gids_arr)->order_by(self::$_default_sort)->get_compiled_select('', false);
            $db->reset_query();
        }
        else
        {
            ...
            $quick_sql = $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::OVERSEA_SUMMARY_LIST_SEARCH_EXPORT)->get();
        ...
        $this->_ci->load->classes('oversea/classes/OverseaHugeSummaryExport');
        $this->_ci->OverseaHugeSummaryExport
        ...
        return $this->_ci->OverseaHugeSummaryExport->run();
    }
    ...
    public function get_cfg() : array
    {
        ...
        $this->_cfg = [
            'title' => [
                    'index',                 'sum_sn',                'bussiness_line',        'sku',
                    'is_refund_tax',         'is_boutique',
                    'purchase_warehouse_id', 'sku_name',              'total_required_qty',
                    'approve_state',         'earliest_exhaust_date',
                    'created_at',            'remark',                'op_name',
            ],
            'select_cols' => [
                    ($this->_ci->oversea_summary->getTable()) => [
                        '*'
                    ],
            ],
            'search_rules' => &$search,
            'droplist' => ['refund_tax', 'oversea_purchase_warehouse', 'oversea_summary_approval', 'boutique_state'],
        ];
        return $this->_cfg;
    }
    ...
    protected function search($params)
    {
        ...
        $o_t = $this->_ci->oversea_summary->getTable();
