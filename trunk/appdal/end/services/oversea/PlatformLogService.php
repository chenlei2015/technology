...
class PlatformLogService
{
    ...
    public function send($params, $context)
    {
        ...
        $row = $this->_ci->oversea_platform_list_log->fetch_table_cols($params);
        return $this->_ci->oversea_platform_list_log->add($row);
    }
    
    public function get_one_listing_log($gid, $offset = 1, $limit = 20)
    {
        $total = 0;
        $rows = $this->_ci->oversea_platform_list_log->get($gid, $total, $offset, $limit);
    ...
    public function multi_send($params)
    {
        ...
        return $this->_ci->oversea_platform_list_log->madd($params);
    }
}
