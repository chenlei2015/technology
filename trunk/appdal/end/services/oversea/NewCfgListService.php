...
class NewCfgListService extends AbstractList
{
    ...
    public function quick_export($gids = '', $profile = '*', $format_type = EXPORT_VIEW_PRETTY, $data_type = VIEW_BROWSER, $charset = 'UTF-8' )
    {
        ...
        if ($gids != '')
        {
            ...
            $quick_sql = $db->from($this->_ci->m_oversea_new->getTable())->where_in('id', $gids_arr)->order_by(self::$_default_sort)->get_compiled_select('', false);
            $db->reset_query();
        }
        else
        {
            ...
            $quick_sql = $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::OVERSEA_NEW_EXPORT)->get();
        ...
        $this->_ci->load->classes('fba/classes/OverseaNewHugeExport');
        $this->_ci->OverseaNewHugeExport
        ...
        return $this->_ci->OverseaNewHugeExport->run();
    ...
    public function get_cfg() : array
    {
        ...
        $this->_cfg = [
            ...
            'select_cols' => [
                ($this->_ci->m_oversea_new->getTable()) => [
                    '*'
                ],
            ],
            'search_rules' => &$search,
    ...
    protected function search($params)
    {
        ...
        $o_t = $this->_ci->m_oversea_new->getTable();
...
//        pr($query->get_compiled_select('',false));exit;
        ...
        if (isset($params['export_save']))
        {
            ...
            $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::OVERSEA_NEW_EXPORT)->set($total.($query_export->get_compiled_select('', false)));
        }
