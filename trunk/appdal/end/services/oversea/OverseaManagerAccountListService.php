...
class OverseaManagerAccountListService extends AbstractList
{
     ...
     * @var string
     */
    protected static $_default_sort = 'station_code desc,platform_code desc';
    ...
    public function get_cfg() : array
    {
        ...
        $this->_cfg = [
            'title' => [
                    'no',               'station_name',     'platform_name',   'manager_name',
                    'manager_state',    'update_info',      'op_name',
            ],
            'select_cols' => [
    ...
    protected function search($params)
    {
        ...
        $o_t = $this->_ci->m_oversea_manager->getTable();
        $p_t = $this->_ci->m_oversea_staff->getTable();
        ...
        if (count($params) == 3)
        {
            return $this->_ci->m_oversea_manager->get_default_station_platform_list($params['page'], $params['per_page']);
        }
        
        $query = $db->from($o_t)->join($p_t, "{$o_t}.gid = {$p_t}.gid");
    ...
    public function translate($search_result)
    {
        ...
        foreach ($search_result['data_list']['value'] as $row => &$val)
        {
            ...
            $val['platform_name'] = INLAND_PLATFORM_CODE[$val['platform_code']]['name'] ?? '-';
