 ...
 * @link
 */
class PrSummaryListService extends AbstractList
{
    /**
     * 默认排序
     * @var string
    ...
    public function importDependent()
    {
        ...
        $this->_ci->load->model('Fba_pr_summary_model', 'm_fba_summary', false, 'fba');
    ...
    {
        $db = $this->_ci->m_fba_summary->getDatabase();
        ...
        if ($gids != '')
        {
            ...
            $quick_sql = $db->from($this->_ci->m_fba_summary->getTable())->where_in('gid', $gids_arr)->order_by(self::$_default_sort)->get_compiled_select('', false);
            $db->reset_query();
        }
        else
        {
            ...
            $quick_sql = $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::FBA_SUMMARY_LIST_SEARCH_EXPORT)->get();
        ...
        $this->_ci->load->classes('fba/classes/FbaHugeSummaryExport');
        $this->_ci->FbaHugeSummaryExport
        ...
        return $this->_ci->FbaHugeSummaryExport->run();
    }
    ...
    public function get_cfg() : array
    {
        ...
        $this->_cfg = [
            'title' => [
                    'index',                 'sum_sn',                'bussiness_line',        'sku',                   'is_refund_tax',
                    'is_boutique',
                    'purchase_warehouse_id', 'sku_name',              'total_required_qty',    'earliest_exhaust_date', 'created_at',
                    'remark',                'op_name',
            ],
            'select_cols' => [
                    ($this->_ci->m_fba_summary->getTable()) => [
                        '*'
                    ],
            ],
            'search_rules' => &$search,
            'droplist' => ['refund_tax', 'fba_purchase_warehouse', 'boutique_state'],
            //'user_profile' => '',
        ];
        return $this->_cfg;
    }
    ...
    protected function search($params)
    {
        ...
        $o_t = $this->_ci->m_fba_summary->getTable();
        ...
        if (isset($params['export_save']))
        {
            ...
            $sql = $query_export->get_compiled_select('', false);
            $this->_ci->load->library('rediss');
            $this->_ci->load->service('basic/SearchExportCacheService');
            $total = str_pad((string)$count, 10, '0', STR_PAD_LEFT);
            $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::FBA_SUMMARY_LIST_SEARCH_EXPORT)->set($total.$sql);
        }
        ...
        //pr($query->get_compiled_select($o_t));exit;
        //pr($query->get());exit;
