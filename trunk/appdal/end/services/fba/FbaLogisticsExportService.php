...
class FbaLogisticsExportService
{
    /**
     * client
     *
    ...
    public function setListingStateTemplate($post)
    {
        $this->_template = $post['template'] ?? __CLASS__;
        $this->_charset   = $post['charset'] ?? 'UTF-8';
        //csv
        $this->_data_type = VIEW_FILE;
        if (!isset($post['gid']))
        {
            //默认选择当前筛选
            $this->_gids = '';
        ...
        //sku,seller_sku,fnsku,asin,account_num、account_id、销售小组、销售名称、销售账号、FBA站点、listing状态（详细）、是否泛欧
        $this->_profile = ['sku','seller_sku','fnsku','asin','account_num','account_id','sale_group_zh_name',
                            'salesman_zh_name','account_name','station_code','listing_state_text','pan_eu','id'];
        $this->_format_type = $post['format_type'] ?? EXPORT_VIEW_PRETTY;
    }
    ...
    public function export_listing_state($file_type = 'csv')
    {
        $this->_file_type = strtolower($file_type);
        if (!in_array($this->_file_type, $this->_allow_file_types))
        {
        ...
        return $this->quick_export_listing_state($this->_gids, $this->_profile, $this->_format_type, $this->_data_type, $this->_charset);
    }
    ...
    public function quick_export($gids = '', $profile = '*', $format_type = EXPORT_VIEW_PRETTY, $data_type = VIEW_BROWSER, $charset = 'UTF-8')
    {
        ...
        if ($gids != '')
        {
            ...
            $quick_sql = $clone_db->from($this->_ci->m_logistics->getTable())->where_in('id', $gids_arr)->order_by('created_at desc, id desc')->get_compiled_select('', false);
        }
        else
        {
            $this->_ci->load->library('rediss');
            $this->_ci->load->service('basic/SearchExportCacheService');
            $quick_sql = $this->_ci->searchexportcacheservice->setScene($this->_ci->searchexportcacheservice::FBA_LOGISTICS_LIST_SEARCH_EXPORT)->get();
...
//            }
        ...
        $this->_ci->load->classes('fba/classes/FbaHugeLogisticsExport');
        $this->_ci->FbaHugeLogisticsExport
        ...
        return $this->_ci->FbaHugeLogisticsExport->run();

    }

    public function quick_export_listing_state($gids = '', $profile = '*', $format_type = EXPORT_VIEW_NATIVE, $data_type = VIEW_BROWSER, $charset = 'UTF-8')
    {
        $this->_ci->load->model('Fba_logistics_list_model', 'm_logistics', false, 'fba');
        $db = $this->_ci->m_logistics->getDatabase();
        $this->_ci->load->dbutil();
        ...
        else
        {
            ...
            $total = substr($quick_sql, 0, 10);
            $quick_sql = substr($quick_sql, 10);
            //$quick_sql = "select * from yibai_fba_logistics_list limit 250000,50000";
            if (!$quick_sql)
            {
                throw new \RuntimeException(sprintf('请选择要导出的资源'));
            }
        ...
        $this->_ci->FbaHugeLogisticsExport
            ...
            ->set_data_sql($quick_sql)
            ->set_export_nums($total)
            ->set_export_type(__FUNCTION__);
        return $this->_ci->FbaHugeLogisticsExport->run_listing_state();
