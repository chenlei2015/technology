 ...
 * @link
 */
class SendRealAllotListService extends AbstractList
{
    /**
     * 默认排序
     * @var string
    ...
    public function importDependent()
    {
        ...
        $this->_ci->load->model('Send_real_allot_model', 'm_send_real', false, 'plan');
    ...
    public function get_cfg() : array
    {
        ...
        $this->_cfg = [
            'title' => [
                'account_name','batch_num','need_batch_weight','order_batch_weight','if_product_attr','only_flag','site',
                  'account_id','seller_sku','erp_sku','sale_30','quantity',
                  'per_weight','total_quantity','fba_available_inv','real_send','by_need','by_order','update_time'
            ],
            'select_cols' => [
                    ($this->_ci->m_send_real->getTable()) => [
                        '*'
                    ],
            ],
            'search_rules' => &$search,
            ...
            'user_profile' => 'python_send_real_allot'
        ];
    ...
    {
        $db = $this->_ci->m_send_real->getDatabase();
        $db->reset_query();
        
        $o_t = $this->_ci->m_send_real->getTable();
