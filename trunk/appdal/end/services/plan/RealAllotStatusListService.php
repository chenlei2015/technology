 ...
 */
class RealAllotStatusListService extends AbstractList
{
    /**
     * 默认排序
     * @var string
    ...
    public function importDependent()
    {
        ...
        $this->_ci->load->model('Real_allot_status_model', 'm_real_allot', false, 'plan');
    ...
    public function get_cfg() : array
    {
        ...
        $this->_cfg = [
            ...
            'select_cols' => [
                    ($this->_ci->m_real_allot->getTable()) => [
                        '*'
                    ],
            ],
            'search_rules' => &$search,
            ...
            'user_profile' => 'python_real_allot_result'
        ];
    ...
    {
        $db = $this->_ci->m_real_allot->getDatabase();
        $db->reset_query();
        
        $o_t = $this->_ci->m_real_allot->getTable();
