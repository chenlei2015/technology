...
class New_cfg extends MY_Controller{
    ...
    public function import()
    {
        ...
        $err_info['errorLines'] = [];
        $inconformity_line_num = $inconformity_data_line_num = $inconformity_format_line_num = $inconformity_site_line_num = [];
        $line_num = 1;
        for ($i = 0; $i < $count; $i++) {
            $insertRows = [];
            foreach ($chunkData[$i] as $k => $value) {
                ...
                if(count($v) < count($modify_item))
                {
                    ...
                    array_push($inconformity_data_line_num,$line_num);
                    continue;
                }
                $row = [];
                ...
                {
                    array_push($err_info['errorLines'],$line_num);
                    array_push($inconformity_site_line_num,$line_num);
                    continue;
                }
                if(preg_match('/^[+-]?[\d]+([\.][\d]+)?([Ee][+-]?[\d]+)?$/', $v[$final_list['erpsku']]))
                {
                    array_push($err_info['errorLines'],$line_num);
                    array_push($inconformity_format_line_num,$line_num);
                    continue;
                }

                foreach ($final_list as $key => $val) {
                    if($key =='erpsku'){
                        $row[$key] = ltrim($v[$val],'_');
                    }else{
                        $row[$key] = $v[$val];
                    }
            ...
            if(!empty($inconformity_data_line_num)){
                array_push($inconformity_line_num,['line_num'=>$inconformity_data_line_num,'reason'=>'字段个数比规定的缺少']);
            }

            if(!empty($inconformity_site_line_num)){
                array_push($inconformity_line_num,['line_num'=>$inconformity_site_line_num,'reason'=>'站点不在要求的范围('.explode(',',array_keys(FBA_STATION_CODE)).')内']);
            }

            if(!empty($inconformity_format_line_num)){
                array_push($inconformity_line_num,['line_num'=>$inconformity_format_line_num,'reason'=>'erpsku格式不符合规定,可能包含科学计数法']);
            }
            ...
            $line_num_info = json_decode($post_result['data_list']['line_num_info']);

            $line_num_info_data = array_merge($inconformity_line_num,$line_num_info);
        }
        $this->data['status'] = 1;
        ...
        $this->data['data_list'] = [
            ...
            'line_num_info' => $line_num_info_data,
        ];
